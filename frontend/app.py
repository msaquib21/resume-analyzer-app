...
/* =====================================================
   ...
   Deep Navy-Black base · Neon Mint primary · Hot Rose danger ·
   Electric Violet prep · Amber Gold score
   ===================================================== */
:root {
  --bg1:  #060914;
  --bg2:  #0A0F1F;
  ...
  /* Unique accent palette */
  --mint:    #00FFA3;   /* PRIMARY  — neon seafoam green */
  --mint-dk: #00CC84;   /* darker variant */
  --rose:    #FF4060;   /* DANGER   — hot coral-red */
  --violet:  #B44DFF;   /* PREP     — electric violet */
  --amber:   #FFB800;   /* SCORE    — warm amber gold */
...
.premium-card:hover {
  ...
  box-shadow:
    0 8px 44px rgba(0,255,163,0.07),
...
div.stFileUploader > label {
  ...
  background: rgba(0,255,163,0.025) !important;
...
/* =====================================================
   7.  CTA BUTTON — UNIQUE DARK + MINT GLOW
   ===================================================== */
div.stButton > button[kind="primary"] {
  ...
  box-shadow:
    0 0 0 1px rgba(0,255,163,0.12) inset,
    0 4px 20px rgba(0,255,163,0.08);
...
div.stButton > button[kind="primary"]:hover {
  ...
  border-color: rgba(0,255,163,0.70) !important;
  transform: translateY(-2px);
  box-shadow:
    0 0 0 1px rgba(0,255,163,0.30) inset,
    0 10px 40px rgba(0,255,163,0.20),
    0 0 80px rgba(0,255,163,0.08) !important;
...
/* =====================================================
   9.  SCORE GAUGE (animated SVG ring)
   ===================================================== */
.score-section {
...
.score-elapsed {
  ...
  color: var(--dimmed); font-family: var(--mono); font-weight: 500;
}
...
[data-baseweb="tab"][aria-selected="true"] {
  ...
  box-shadow: 0 0 0 1px rgba(0,255,163,0.22) inset !important;
}
...
.result-scroll::-webkit-scrollbar-thumb {
  background: rgba(0,255,163,0.22); border-radius: 999px;
}
...
/* =====================================================
   12. GAP CARDS — Hot Rose accent, compact layout
   ===================================================== */
.gap-card {
  padding: 12px 16px;
  border-radius: var(--radius-sm);
...
.gap-skill-name {
  font-weight: 800; font-size: 0.92rem; color: var(--text); letter-spacing: -0.01em;
}
.gap-body {
  color: var(--muted); font-size: 0.85rem; line-height: 1.5;
  padding-left: 32px; white-space: normal;
}
.gap-jd-req {
  margin-top: 6px;
  ...
  border: 1px solid rgba(0,255,163,0.14);
  font-size: 0.80rem; color: #A8FFD8;
}
.jd-req-label { font-weight: 700; color: var(--mint); }

/* =====================================================
   13. IMPROVEMENT CARDS — Mint accent, compact focus
   ===================================================== */
.imp-card {
...
.imp-card:hover {
  background: rgba(0,255,163,0.025);
  border-color: rgba(0,255,163,0.14);
  transform: translateX(4px);
}
.imp-check {
  ...
  color: var(--mint); font-size: 0.80rem; font-weight: 800; flex-shrink: 0;
}
.imp-content { flex: 1; }
.imp-num {
  font-weight: 800; font-size: 0.74rem; color: var(--mint);
  font-family: var(--mono); letter-spacing: 0.03em; margin-bottom: 2px;
}
.imp-text {
...
.imp-meta-tag {
  ...
  margin-top: 6px; padding: 2px 8px; border-radius: 6px;
...
/* =====================================================
   14. PREPARATION TIMELINE — Electric Violet accent
   ===================================================== */
.timeline { position: relative; padding-left: 22px; }
.timeline::before {
  content: ''; position: absolute;
  left: 9px; top: 18px; bottom: 18px; width: 2px;
  background: linear-gradient(180deg, rgba(180,77,255,0.45) 0%, rgba(180,77,255,0.06) 100%);
  border-radius: 999px;
}
.prep-card {
  position: relative;
...
.prep-card:hover {
  ...
  border-color: rgba(180,77,255,0.16);
  transform: translateX(4px);
}
.prep-dot {
  ...
  box-shadow: 0 0 0 3px rgba(180,77,255,0.22);
}
.prep-header {
...
.prep-num {
  ...
  color: var(--violet); padding: 2px 8px; border-radius: 6px;
  background: rgba(180,77,255,0.10); border: 1px solid rgba(180,77,255,0.20);
}
.prep-gap-title {
  font-weight: 800; font-size: 0.90rem; color: #E0CCFF; letter-spacing: -0.01em;
}
...
.prep-tag {
  ...
  width: fit-content; padding: 2px 6px; border-radius: 4px;
}
...
hr {
  ...
  background: linear-gradient(90deg, transparent, rgba(0,255,163,0.12), transparent) !important;
...
.loading-spinner {
  ...
  border: 3px solid rgba(0,255,163,0.08);
...
.loading-stages {
  ...
  margin-top: 1.6rem; width: 100%; max-width: 340px;
}
.loading-stage {
  display: flex; align-items: center; gap: 10px;
  font-size: 0.8rem; padding: 9px 13px; border-radius: 9px;
  background: rgba(255,255,255,0.018);
  border: 1px solid rgba(255,255,255,0.05);
  animation: fadeSlideUp 0.4s ease-out both;
}
.loading-stage:nth-child(1) { animation-delay: 0.0s; }
.loading-stage:nth-child(2) { animation-delay: 0.25s; }
.loading-stage:nth-child(3) { animation-delay: 0.5s; }
.loading-stage:nth-child(4) { animation-delay: 0.75s; }
.stage-dot { width: 8px; height: 8px; border-radius: 50%; }
.stage-text { color: var(--muted); font-family: var(--mono); font-weight: 500; }

/* =====================================================
   19. TAB SECTION HEADERS
   ===================================================== */
.tab-header {
  display: flex; align-items: center; gap: 10px;
...
/* =====================================================
   20. STREAMLIT-SPECIFIC OVERRIDES (critical)
   ===================================================== */
...
/* Remove white column backgrounds */
[data-testid="column"] {
  background: transparent !important;
}
...
/* Fix stTextArea inner container showing white/grey */
...
/* Fix file uploader container */
[data-testid="stFileUploader"] {
  background: transparent !important;
}
[data-testid="stFileUploaderDropzone"] {
  background: rgba(0,255,163,0.025) !important;
  border: 2px dashed rgba(0,255,163,0.25) !important;
  border-radius: 12px !important;
}
...
/* Fix divider in left panel */
[data-testid="stDivider"] hr {
  border-color: rgba(255,255,255,0.06) !important;
}

/* Make Streamlit error box blend with dark theme */
[data-testid="stAlert"] {
  background: rgba(255,64,96,0.06) !important;
  border: 1px solid rgba(255,64,96,0.22) !important;
  border-radius: 12px !important;
...
st.markdown(
    ...
    <div class="hero-container">
      ...
      <div class="hero-subtitle">
        <span class="hero-badge">LOCAL</span>
        Ollama · LangGraph · ChromaDB · sentence-transformers — no API keys needed
      </div>
    </div>
    """,
...
with col_left:
    ...
    st.markdown(
        ...
        <div class="section-label">
          <span class="label-icon label-icon-jd">📋</span>
          Job Description
        </div>
        """,
        unsafe_allow_html=True,
    ...
    job_description = st.text_area(
        "Job Description",
        height=240,
        placeholder="Paste the full Job Description here — skills, tools, responsibilities…",
    ...
    st.markdown(
        """
        <div class="section-label">
          <span class="label-icon label-icon-pdf">📄</span>
          Resume PDF
        </div>
        """,
        unsafe_allow_html=True,
    ...
    resume_file = st.file_uploader(
        ...
        help="Upload a PDF resume. Everything runs 100% locally on your machine.",
        label_visibility="collapsed",
    )
...
with col_right:
        ...
        st.markdown(
            ...
            <div class="onboarding">
              ...
              <div class="onboarding-steps">
                <div class="ob-step">
                  <div class="ob-num ob-num-1">1</div>
                  <div class="ob-step-content">
                    <div class="ob-step-title">Paste the Job Description</div>
                    <div class="ob-step-desc">Copy the JD from the job listing — include skills, tools, and requirements for best results.</div>
                  </div>
                </div>
                <div class="ob-step">
                  <div class="ob-num ob-num-2">2</div>
                  <div class="ob-step-content">
                    <div class="ob-step-title">Upload your Resume</div>
                    <div class="ob-step-desc">Drop your resume as a PDF. The AI will parse every section and extract your skills automatically.</div>
                  </div>
                </div>
                <div class="ob-step">
                  <div class="ob-num ob-num-3">3</div>
                  <div class="ob-step-content">
                    <div class="ob-step-title">Get Instant Analysis</div>
                    <div class="ob-step-desc">Receive a match score, identified gaps, actionable improvements, and interview prep — all locally.</div>
                  </div>
                </div>
              </div>
              <div class="ob-tip">
                ...
                <div class="ob-tip-text">
                  <strong>Pro tip:</strong> Include specific tools, frameworks, and years of experience in the JD for more precise gap detection and targeted improvement suggestions.
                </div>
              </div>
            </div>
            """,
    ...
    if not job_description.strip():
        ...
        def _loading_html(stage_key: str, message: str) -> str:
            ...
            all_stages = [
                ("extracting",     "Parsing PDF & embeddings"),
            ...
            for i, (skey, slabel) in enumerate(all_stages):
                if i < current_idx:
                    dot_style = "background:#34D399;"
                    text_style = "color:#34D399;"
                elif i == current_idx:
                    dot_style = "background:#818CF8; animation:dotPulse 1.2s ease-in-out infinite;"
                    text_style = "color:#F1F5F9;"
                else:
                    dot_style = "background:rgba(255,255,255,0.12);"
                    text_style = "color:#64748B;"
            ...
            <div class="loading-container">
              ...
              <div class="loading-sub">Running local pipeline — no data leaves your machine</div>
              <div class="loading-stages">{stages_html}</div>
            </div>
            """
        ...
        try:
                ...
                for raw_line in resp.iter_lines():
                    ...
                    if event_type == "result":
                        ...
                        loading_placeholder.markdown(
                            _loading_html(event_type, event_msg),
                            unsafe_allow_html=True,
                        )
            ...
            st.session_state.error = "Cannot reach the backend at http://localhost:8000 — is it running?"
        except requests.exceptions.RequestException as exc:
            st.session_state.error = f"Backend request failed: {exc}"
        ...
        if st.session_state.error:
            ...
            if score >= 8:
                ring_color = "#00FFA3"
                score_fg = "#00FFA3"
                glow_bg = "rgba(0,255,163,0.10)"
            elif score >= 5:
                ring_color = "#FFB800"
                score_fg = "#FFB800"
                glow_bg = "rgba(255,184,0,0.10)"
            else:
                ring_color = "#FF4060"
                score_fg = "#FF4060"
                glow_bg = "rgba(255,64,96,0.10)"
            ...
            st.markdown(
                ...
                <div class="score-section">
                  <div class="score-ring-wrap">
                    <div class="score-ring-glow" style="background: radial-gradient(circle, {glow_bg}, transparent 70%);"></div>
                    ...
                    <div class="score-center">
                      <div class="score-value" style="color: {score_fg};">{score}<span style="font-size:1.2rem; color: var(--dimmed);">/10</span></div>
                      <div class="score-label">Match Score</div>
                    </div>
                  </div>
                  <div class="score-elapsed">completed in {elapsed:.1f}s</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
            ...
            # ── Tabs ──────────────────────────────────────────────────
            gaps_tab, imp_tab, prep_tab = st.tabs(
                ["🔍  Identified Gaps", "⚡  Improvements", "🎯  Preparation"]
            )
            ...
            with gaps_tab:
                st.markdown(
                    ...
                    <div class="tab-header">
                      <div class="tab-header-icon gaps">🔍</div>
                      <div class="tab-header-title">Identified Skill Gaps</div>
                      <div class="tab-header-count">{len(gaps)} found</div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
                ...
                if gaps:
                    ...
                    for i, g in enumerate(gaps, start=1):
                        if score < 5:
                            ...
                            badge_class = "gap-badge-high"
                        elif score < 8:
                            sev_class = "severity-med"
                            badge_class = "gap-badge-med"
                        else:
                            sev_class = "severity-low"
                            badge_class = "gap-badge-low"

                        text = g.strip()
                        # Split "Skill Name: explanation text"
                        colon_idx = text.find(":")
                        ...
                        # Split off JD requirement if model included it
                    ...
                    st.markdown(
                        '<div class="empty-state"><div class="empty-state-icon">✅</div><div class="empty-state-text">No gaps detected — great match!</div></div>',
                        unsafe_allow_html=True,
                    )
            ...
            with imp_tab:
                st.markdown(
                    f'<div class="tab-header"><div class="tab-header-icon imps">⚡</div><div class="tab-header-title">Actionable Improvements</div><div class="tab-header-count">{len(improvements)} items</div></div>',
                    unsafe_allow_html=True,
                )
                ...
                if improvements:
                    ...
                    for i, imp in enumerate(improvements, start=1):
                        text = imp.strip()
                        text = re.sub(r"^Add bullet:\s*", "", text, flags=re.IGNORECASE)
                        text = imp.strip()
                        ...
                        # Fallback if model outputted unpiped or plain imperative text
                        ...
                        st.markdown(
                            f'<div class="imp-card"><div class="imp-check">✓</div><div class="imp-content"><div class="prep-header"><div class="imp-num">#{i:02d}</div>{area_html}</div><div class="imp-text" style="font-weight: 600; color: #FFFFFF;">{action_val}</div>{align_html}</div></div>',
                            unsafe_allow_html=True,
                        )
                    st.markdown("</div>", unsafe_allow_html=True)
                else:
                    st.markdown(
                        '<div class="empty-state"><div class="empty-state-icon">🎉</div><div class="empty-state-text">No improvements needed — your resume is strong!</div></div>',
                        unsafe_allow_html=True,
                    )
            ...
            with prep_tab:
                st.markdown(
                    f'<div class="tab-header"><div class="tab-header-icon prep">🎯</div><div class="tab-header-title">Interview Preparation</div><div class="tab-header-count">{len(preparation)} steps</div></div>',
                    unsafe_allow_html=True,
                )

                if preparation:
                    st.markdown(
                        '<div class="result-scroll"><div class="timeline">',
                        unsafe_allow_html=True,
                    )
                    for i, p in enumerate(preparation, start=1):
                        ...
                        if gap_val or study_val or practice_val or angle_val:
                            ...
                            card_inner = f'<div class="prep-header"><div class="prep-num">Step {i}</div></div><div class="prep-text">{text}</div>'

                        st.markdown(
                            f'<div class="prep-card"><div class="prep-dot"></div>{card_inner}</div>',
                            unsafe_allow_html=True,
                        )
                    ...
                    st.markdown(
                        ...
                        <div class="empty-state">
                          ...
                          <div class="empty-state-text">No preparation topics generated.</div>
                        </div>
                        """,
                        unsafe_allow_html=True,
                    )
